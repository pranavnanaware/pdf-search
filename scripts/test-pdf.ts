...
import { getCachedResult, setCachedResult, closeRedisConnection } from '../lib/redis.ts';
...
// Main test function
async function testPdfProcessing(url: string, query: string) {
...
// Get URL and query from command line arguments
const url = process.argv[2];
const query = process.argv[3];

if (!url || !query) {
  console.error('Please provide a URL and query as arguments:');
  console.error('npm run test-pdf <url> <query>');
  process.exit(1);
}

// Run the test
testPdfProcessing(url, query)
  .then(async () => {
